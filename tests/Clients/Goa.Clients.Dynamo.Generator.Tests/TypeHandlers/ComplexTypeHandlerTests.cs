...
public class ComplexTypeHandlerTests
{
    ...
    public async Task GenerateFromDynamoRecord_StringStringDictionary_ShouldGenerateCorrectCode()
    {
        ...
        // Assert
        var expected = "(record.TryGetStringDictionary(\"Metadata\", out var metadata) ? metadata : null ?? new Dictionary<string, string>())";
        await Assert.That(result)
            .IsEqualTo(expected);
    }
    ...
    public async Task GenerateFromDynamoRecord_StringIntDictionary_ShouldGenerateCorrectCode()
    {
        ...
        // Assert
        var expected = "(record.TryGetStringIntDictionary(\"Scores\", out var scores) ? scores : null ?? new Dictionary<string, int>())";
        await Assert.That(result)
            .IsEqualTo(expected);
    }
